...
| | |
|---|---|
| **What it is** | An autonomous, multi-agent AI system that converts a plain-English research brief into a cited, consulting-grade market report |
| **Who it's for** | Analysts, founders, and decision-makers who need fast, evidence-backed market intelligence without doing the research manually |
| **Core loop** | Brief in → 7 AI agents plan, search, extract, validate, and write → cited report out |
| **Status** | Deployed — frontend live on Vercel, backend live, full pipeline operational end-to-end |

</div>

Meridian compresses a process that normally takes a human analyst hours or days — scoping a question, searching the web, reading sources, extracting evidence, checking it's credible, and writing it up — into a single automated pipeline that runs in one session, with every claim traceable back to a live source.

<br/>

## Business Problem

Doing market research properly is slow and expensive. Getting a credible answer to a question like *"What does the competitive landscape for X look like?"* typically means:

- Manually running many search queries across the web
- Reading and cross-referencing sources to separate signal from noise
- Tracking, by hand, which claim came from which source
- Writing everything up into a structured report — usually the job of a paid analyst or consulting engagement
...
Meridian's goal is to be a **self-serve AI research analyst**: a user submits a brief and, without any further manual work, receives a structured report where every key finding can be traced back to the extracted evidence and the original web source that supports it.

Specifically, the product aims to:
- Turn an unstructured brief into a scoped, actionable research plan automatically
- Source evidence from the live web rather than relying on stale training data
...
A plain-English walkthrough of what happens between a user submitting a brief and receiving a report:

| # | Agent | In plain English |
|:---:|---|---|
| 1 | **Planner** | Reads the brief and breaks it into a handful of concrete, searchable research questions. |
| 2 | **Research** | Runs a live web search for each question and collects candidate sources. |
| 3 | **Extraction** | Reads each source and pulls out specific, quotable claims — not just a summary. |
| 4 | **Validation** | Re-checks each claim against its source and flags how confident it is in that claim. |
| 5 | **Citation** | Formats every source into a clean, referenceable citation. |
| 6 | **Report** | Weaves the validated evidence into a structured report with an executive summary and key findings. |
| 7 | **Linker** | Goes back through the finished report and wires every finding to the exact evidence and citation behind it. |

This is the "explain it to a stakeholder" version of the same flow detailed technically in [The Multi-Agent Research Pipeline](#the-multi-agent-research-pipeline) above.

<br/>

## Evaluation & Reliability

Meridian is designed so a report is never presented as more reliable than the evidence actually is:

| Mechanism | What it does |
|---|---|
| **Fail-fast pipeline** | If any stage returns an empty result (no tasks, no sources, no evidence), the job is marked `failed` immediately rather than producing a report with the gaps papered over. |
| **Dedicated Validation stage** | Every piece of extracted evidence is independently re-checked against its source before it can enter the report, rather than trusting extraction output blindly. |
...
| Metric | Value |
|---|---|
| Average end-to-end pipeline runtime | *e.g. ~45–90s per brief* |
| Average sources found per job | *e.g. 8–12* |
| Average evidence items extracted per job | *e.g. 15–25* |
| Report generation success rate | *e.g. 95% of submitted briefs* |
| LLM calls per job | *e.g. ~10–15 (Planner + Extraction + Validation + Report)* |
...
- **Synchronous pipeline** — `POST /api/research/` runs all seven agent stages inline and only responds once the job finishes, so a large brief keeps the request open for the full pipeline duration instead of streaming progress from the backend.
- **Single LLM provider** — the pipeline depends entirely on Gemini; there's no automatic fallback if the provider is rate-limited or unavailable.
- **Search coverage is Tavily-bound** — research quality is capped by what Tavily's search index surfaces for a given query.
...
- Move job execution to a background worker/queue and stream progress to the frontend (e.g. via WebSockets or polling), instead of blocking on one long HTTP request.
- Parallelize independent research tasks to cut total pipeline runtime.
- Put the `memory_records` / `pgvector` table to use — cache or semantically recall evidence from prior jobs to speed up and enrich future research.
- Add a fallback LLM provider for resilience against Gemini outages or rate limits.
- Add an automated evaluation harness to track citation accuracy and hallucination rate over time.
- Add report export (PDF/Word) and shareable read-only report links.
- Support team workspaces so research jobs can be shared across users, not just owned individually.
